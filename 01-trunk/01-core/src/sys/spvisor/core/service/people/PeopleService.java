...
public class PeopleService {
	 ...
	 * //查询当前人员动态表
	 * 
	 * @Transactional public List<PeopleToday> selectByExample(PeopleToday msg)
	 * {
	 * 
	 * TUserExample userExample = new TUserExample();
	 * sys.spvisor.core.entity.ana.TUserExample.Criteria userCriteria =
	 * userExample.createCriteria();
	 * 
	 * if(msg.getUserLoginName() == "" || msg.getUserLoginName() == null){}
	 * else{ userCriteria.andUserLoginNameLike("%" +msg.getUserLoginName()
	 * +"%");//姓名 } if(msg.getUserName()=="" || msg.getUserName()==null){} else{
	 * userCriteria.andUserNameLike("%" +msg.getUserName() +"%");//出差位置 }
	 * if(msg.getUserWorkStatusId() == null) {} else {
	 ...
	 * 
	 * // List<PeopleToday> people return null; }
	 */
	...
	public List<TUser> queryPeopleUser(PeopleManageCriteria params) {
		...
		List<TUser> temp = TUserMapper.queryUserByCondition(params);
		if (params.getUserWorkStatusId() == null) {
			List<TUser> lists = new ArrayList<TUser>();
			for (TUser user : temp) {
				if (user.getUserWorkStatusId() != 7) {
					lists.add(user);
				}
			}
			return lists;
